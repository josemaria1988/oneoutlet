                                    ...
                                    <script type="text/javascript">console.log("{{$cartdata}}");</script>
                                        ...
                                        <div class="d-flex pb-3 border-bottom mb-3">
                                            <div class="col-8 px-0 ">
                                                ...
                                                <ul class="p-0 text-muted list-font mb-1"><li>
                                                    @if ($cart->variants_id == null)
                                                        ${{number_format((float)($cart->item_price), 2, '.'. '')}}<?php $sub_total += floatval($cart->item_price); ?>
                                                    @else
                                                        {{$cart->variants_name}} : ${{number_format((float)($cart->variants_price), 2, '.'. '')}}<?php $sub_total += floatval($cart->variants_price); ?>
                                                    @endif
                                                    </li></ul>
                                                @if ($cart->extras_id != null)
                                                    ...
                                                    <span class="badge primary-outline">
                                                        {{$name}} : ${{$extra_prices[$key]}}<?php $sub_total += floatval($extra_prices[$key]); ?>
                                                    </span>
                                                    @endforeach
                                                @endif
                                            ...
                                            <div class="col-4 px-0 d-grid justify-items-end">
                                                ...
                                                <p class=""><?php $sub_total *= floatval($cart->qty); $total += $sub_total; ?>${{number_format($sub_total, 2, '.', '')}}</p>
                                            </div>
                                        </div>
                                        @endforeach
                                        <div class="d-flex">
                                            ...
                                            <div class="col-4 px-0">
                                                <h6 class="text-success text-right">${{number_format($total, 2, '.', '')}}</h6>
                                            </div>
                                        </div>
