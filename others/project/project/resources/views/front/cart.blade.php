...
<section class="cart">
    ...
    <form class="form-horizontal" method="POST" action="{{$user->url}}" id="payment-form" validate>@csrf
        ...
        <input type='hidden' id="notify_url" name='notify_url' value="{{ URL::to('/orders/notify') }}">
        <input type='hidden' id="cancel_return" name='cancel_return' value="{{ URL::to('/orders/cancel') }}">
        <input type='hidden' name='return' id="return_pay" value="{{ URL::to('/orders/success/') }}">
        <input name="txn_type" type="hidden" value="web_accept" />
        <input name="txn_id" type="hidden" value="CUSTOMER" />
        <input type="hidden" name="cmd" value="_xclick">
        <button type="hidden" name="pay_now" id="btn_paypal" class="btn btn-raised btn-success btn-min-width" disabled>ADQUIRIR</button>
    </form>
